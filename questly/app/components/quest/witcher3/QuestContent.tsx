...
export function QuestContent({ title, shortDesc, tags, searchTags, completed, search }: Props) {
	...
	return (
				...
				{tags.map((tag) => (
					<span
						...
						onClick={(e) => {
							...
							const isActive = filters.search === tag && filters.searchTags;

							setFilters({
								...filters,
								searchTags: isActive ? false : true,
								search: isActive ? "" : tag
							});
						}}
						className={`
		...
		${
			filters.search.toLowerCase() === tag.toLowerCase() && filters.searchTags
				? `
					border-[#e6c36a]
					bg-linear-to-b from-[#4a3f1f] to-[#2a2412]
					text-white
